...
const bookFilesUpload = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      cb(null, file.fieldname === 'cover' ? COVERS_DIR : BOOKS_DIR);
    },
    filename: (req, file, cb) => {
      if (file.fieldname === 'cover') {
        const ext = path.extname(file.originalname).toLowerCase() || '.jpg';
        return cb(null, generateStoredName(ext));
      }
      cb(null, generateStoredName('.pdf'));
    },
  }),
  limits: { fileSize: PDF_MAX_BYTES },
  fileFilter: (req, file, cb) => {
    if (file.fieldname === 'pdf') return pdfFileFilter(req, file, cb);
